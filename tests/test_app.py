...
def test_stop_recording(client):
    ...
    response = client.post(
        "/stop-recording", json={"action": "stop", "filename": "test.wav"}
    )
    assert response.status_code == 200
...
def test_generate_spectrogram_invalid_file(client):
    response = client.post(
        "/generate-spectrogram", json={"audioFileURL": "non_existent_file.wav"}
    )
...
def test_get_transcript_whisper_invalid_file(client):
    response = client.post(
        "/get-transcript-whisper", json={"fileName": "non_existent_file.wav"}
    )
    assert response.status_code == 500
    assert "error" in response.get_json()
